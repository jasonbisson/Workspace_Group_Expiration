...
def get_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='Google Group Membership expiration arguments.')
    parser.add_argument(
        "--group_id", help="The email address of Google Group.", required=True)
    parser.add_argument(
        "--member_key", help="The email address of the Google user.", required=True)
    parser.add_argument(
        "--hours", type=int, help="Hours until expiration of Group membership.", default=8)
...
def create_google_group_membership(service, group_id, member_key, required_date_format):
    ...
    try:
        ...
        response = service.groups().memberships().create(
            parent=groupName, body=membership).execute()
        print(response)
...
def main():
    ...
    create_google_group_membership(
        service, args.group_id, args.member_key, required_date_format)
